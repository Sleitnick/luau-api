...
class MarkdownBuilder:
	...
	def append_signature(self, sig: str, stack: str):
		self.builder.append(
			f'<span class="signature">`{sig}`</span>\n<span class="stack">`[{stack}]`</span>'
		)
...
class MarkdownRefAndMeta:
	...
	def __init__(
		self, markdown: str, name: str, ret: str, stack: str, args: list[RefArg]
	):
...
def read_markdown_file_and_metadata(filepath: str) -> MarkdownRefAndMeta:
	yaml_lines = []
	md_lines = []
	...
	return MarkdownRefAndMeta(
		...
		ret=meta["ret"],
		stack=meta["stack"],
		args=list(map(lambda a: RefArg(a), meta["args"]))
		if meta["args"] is not None
		else list(),
	)


def build_signature(meta: MarkdownRefAndMeta) -> str:
	args = ""
	if len(meta.args) > 0:
		args = ", ".join(
			map(
				lambda arg: arg.arg_type if arg.fn else arg.arg_type + " " + arg.name,
				meta.args,
			)
		)
...
def gen():
	...
	with open("./ref/ref.yaml") as ref_stream:
		...
		for section in sections:
			...
			for md_filepath in section["functions"]:
				...
				if len(meta.args) > 0:
					builder.append(
						"\n".join(
							map(lambda arg: f"- `{arg.name}`: {arg.desc}", meta.args)
						)
					)
...
def watch_and_gen():
	event_handler = ChangeHandler(safe_gen)
	observer = Observer()
	_ = observer.schedule(event_handler, "./ref", recursive=True)
	observer.start()
